      ...
      <div className="glass border border-indigo-500/25 rounded-2xl p-4 flex flex-wrap gap-3 items-end">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl text-xs text-gray-400 glass border border-white/10 hover:text-white transition-all"
          >
            <FiX size={12} />
          </button>
        </div>
    ...
    <div className="glass border border-indigo-500/25 rounded-2xl p-4 flex flex-wrap gap-3 items-end">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="px-3 py-2 rounded-xl text-xs text-gray-400 glass border border-white/10 hover:text-white"
        >
          <FiX size={12} />
        </button>
      </div>
...
export default function StatsPanel({ showToast }) {
  ...
  const handleAdd = async (form) => {
    try {
      const { data } = await API.post("/stats", form);
      setStats((s) => [...s, data]);
  ...
  const handleSave = async (id, form) => {
    try {
      const { data } = await API.put(`/stats/${id}`, form);
      setStats((s) => s.map((x) => (x._id === id ? data : x)));
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setStats((s) => s.filter((x) => x._id !== id));
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-0.5">
            These appear as the stat cards in the About section (Years Exp.,
            Projects Done, etc.)
          </p>
        </div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="h-16 glass border border-white/8 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : (

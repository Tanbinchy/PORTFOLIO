    ...
    return (
      <div className="flex flex-wrap gap-2 items-end p-3 glass border border-indigo-500/20 rounded-xl">
        <input
          ...
          className="glass-input flex-1 min-w-[120px] px-3 py-2 rounded-lg text-xs"
        />
        <input
          type="number"
          value={form.level}
          ...
          min={0}
          max={100}
          className="glass-input w-16 px-3 py-2 rounded-lg text-xs"
        />
        <select
          value={form.badge}
        ...
        <div className="flex gap-1">
          <button
            ...
            className="px-3 py-2 rounded-lg bg-indigo-500 text-white text-xs"
          >
            <FiSave size={11} />
          </button>
          <button
            onClick={() => setEditing(false)}
            className="px-3 py-2 rounded-lg glass border border-white/10 text-gray-400 text-xs"
          >
            <FiX size={11} />
          </button>
        </div>
    ...
    <div className="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/3 group transition-all">
      ...
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        <button
          ...
          className="w-6 h-6 rounded flex items-center justify-center text-gray-600 hover:text-indigo-400 hover:bg-indigo-500/10 transition-all"
        >
          <FiEdit2 size={11} />
        </button>
        <button
...
function CategoryCard({ cat, onSave, onDelete }) {
  ...
  const updateSkill = (oldName, updated) => {
    setForm((f) => ({
      ...f,
      skills: f.skills.map((s) => (s.name === oldName ? updated : s)),
    }));
  };
      ...
      <div className="flex items-center gap-3 p-4">
        ...
        <div className="flex-1">
          ...
          <div className="text-gray-600 text-xs">
            {cat.skills?.length || 0} skills · {cat.colorTheme}
          </div>
        </div>
        ...
        <div className="border-t border-white/6 p-4 space-y-3">
          ...
          {editing && (
            <div className="glass border border-indigo-500/20 rounded-xl p-4 space-y-3 mb-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {EMOJIS.map((e) => (
                      <button
                        ...
                        className={`w-8 h-8 rounded-lg text-base transition-all ${form.emoji === e ? "bg-indigo-500/20 border border-indigo-500/40" : "hover:bg-white/8"}`}
                      >
                        {e}
                      </button>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-1">
                    {THEMES.map((t) => (
                      <button
                        key={t}
                        onClick={() =>
                          setForm((f) => ({ ...f, colorTheme: t }))
                        }
                        className={`px-2 py-1 rounded text-[10px] font-semibold capitalize transition-all ${form.colorTheme === t ? "bg-indigo-500/25 text-indigo-300 border border-indigo-500/40" : "text-gray-500 glass border border-white/8 hover:text-white"}`}
                      >
                        {t}
                      </button>
                    ))}
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => setEditing(false)}
                  className="px-3 py-2 rounded-xl text-xs text-gray-400 glass border border-white/10"
                >
                  <FiX size={12} />
                </button>
              </div>
          ...
          {(form.skills || []).map((skill) => (
            <SkillItem
              ...
              catId={cat._id}
              onUpdate={(updated) => {
                updateSkill(skill.name, updated);
              }}
              onDelete={(name) => {
                deleteSkill(name);
              }}
          ...
          {addingSkill ? (
            <div className="flex flex-wrap gap-2 items-end p-3 glass border border-indigo-500/20 rounded-xl">
              <input
                value={newSkill.name}
                onChange={(e) =>
                  setNewSkill((f) => ({ ...f, name: e.target.value }))
                }
                placeholder="Skill name"
                className="glass-input flex-1 min-w-[120px] px-3 py-2 rounded-lg text-xs"
                autoFocus
              ...
              <input
                ...
                onChange={(e) =>
                  setNewSkill((f) => ({ ...f, level: +e.target.value }))
                }
                min={0}
                max={100}
                className="glass-input w-16 px-3 py-2 rounded-lg text-xs"
              ...
              <select
                ...
                onChange={(e) =>
                  setNewSkill((f) => ({ ...f, badge: e.target.value }))
                }
                className="glass-input px-3 py-2 rounded-lg text-xs"
              >
                {BADGES.map((b) => (
    ...
    <div className="glass border border-indigo-500/25 rounded-2xl p-5 space-y-4">
        ...
        <div className="flex flex-wrap gap-1">
          {EMOJIS.map((e) => (
            <button
              key={e}
              onClick={() => setForm((f) => ({ ...f, emoji: e }))}
              className={`w-9 h-9 rounded-xl text-lg transition-all ${form.emoji === e ? "bg-indigo-500/20 border border-indigo-500/40" : "hover:bg-white/8 glass border border-white/6"}`}
            >
              {e}
            </button>
          ))}
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl text-sm text-gray-400 glass border border-white/10"
        >
          <FiX size={14} />
        </button>
      </div>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-0.5">
            Manage skill categories and individual proficiency levels
          </p>
        </div>
        {/* <button
          onClick={handleSeed}
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="h-20 glass border border-white/8 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : (
